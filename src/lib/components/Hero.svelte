<script lang="ts">
	...
	let domTitle: HTMLElement | undefined = $state();
...
{#if ready}
	<section
		class="relative z-10 flex flex-col items-center justify-center min-h-svh text-center px-4"
	>
		<div
      ...
      onintroend={animateTitle}
			class="font-archivo font-bold text-[clamp(3rem,-0.3137rem+13.5948vw,12rem)] text-white mb-6 uppercase whitespace-pre-wrap"
			style="text-shadow: 0 0 40px rgba(120, 0, 255, 0.2);"
		>
			<span bind:this={domTitle} aria-hidden="true" class="text-cursor" class:finished={isDone}></span>
			<span class="sr-only">CARLOS S.</span>
		</h1>
		...
		<div
			...
			class="flex flex-col sm:flex-row gap-4 w-6/7 sm:w-full justify-center"
		>
			<a
				href="#projetos"
...
<style>
	...
	.text-cursor::after {
		...
		border-right: 0.08em solid var(--color-violet-primary);
		margin-left: 0.08em;
		animation: blink 1.25s step-end infinite;
	}

  .text-cursor.finished::after {
    animation: none;
    opacity: 0;
